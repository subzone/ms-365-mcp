...
[![npm version](https://img.shields.io/npm/v/@subzone81/ms-365-mcp.svg)](https://www.npmjs.com/package/@subzone81/ms-365-mcp) [![build status](https://github.com/subzone/ms-365-mcp/actions/workflows/build.yml/badge.svg)](https://github.com/subzone/ms-365-mcp/actions/workflows/build.yml) [![license](https://img.shields.io/badge/license-MIT-blue.svg)](https://github.com/subzone/ms-365-mcp/blob/main/LICENSE)
...
```json
{
  "mcpServers": {
    "ms365": {
      "command": "npx",
      "args": ["-y", "@subzone81/ms-365-mcp", "--toon"]
    }
  }
}
```
...
```bash
MS365_MCP_OUTPUT_FORMAT=toon npx @subzone81/ms-365-mcp
```
...
npx @subzone81/ms-365-mcp --list-permissions

# Organization mode (includes Teams, SharePoint, etc.)
npx @subzone81/ms-365-mcp --org-mode --list-permissions

# Filtered by preset
npx @subzone81/ms-365-mcp --preset mail --list-permissions
```
...
```json
{
  "mcpServers": {
    "ms365": {
      "command": "npx",
      "args": ["-y", "@subzone81/ms-365-mcp", "--org-mode"]
    }
  }
}
```
...
```json
{
  "mcpServers": {
    "ms365-china": {
      "command": "npx",
      "args": ["-y", "@subzone81/ms-365-mcp", "--org-mode", "--cloud", "china"]
    }
  }
}
```
...
```bash
claude mcp add ms365 -- npx -y @subzone81/ms-365-mcp
```
...
```bash
# macOS/Linux
claude mcp add ms365 -- npx -y @subzone81/ms-365-mcp --org-mode

# Windows (use cmd /c wrapper)
claude mcp add ms365 -s user -- cmd /c "npx -y @subzone81/ms-365-mcp --org-mode"
```
...
```bash
# macOS/Linux
claude mcp add ms365-china -- npx -y @subzone81/ms-365-mcp --org-mode --cloud china

# Windows (use cmd /c wrapper)
claude mcp add ms365-china -s user -- cmd /c "npx -y @subzone81/ms-365-mcp --org-mode --cloud china"
```
...
```json
{
  "mcpServers": {
    "ms365": {
      "command": "node",
      "args": ["/absolute/path/to/ms-365-mcp/dist/index.js", "--org-mode"]
    }
  }
}
```
...
- **CLI login**:
  ```bash
  npx @subzone81/ms-365-mcp --login
  ```
...
```bash
npx @subzone81/ms-365-mcp --http 3000
```
...
If you are running ms-365-mcp as part of a larger system that manages Microsoft OAuth tokens externally, you can
provide an access token directly to this MCP server:

```bash
MS365_MCP_OAUTH_TOKEN=your_oauth_token npx @subzone81/ms-365-mcp
```
...
```bash
npx @subzone81/ms-365-mcp --list-accounts
```
...
MS365_MCP_EXPECTED_USERNAME=work@company.com npx @subzone81/ms-365-mcp --login

# Or pin the exact MSAL homeAccountId shown by --list-accounts
npx @subzone81/ms-365-mcp --expected-home-account-id <homeAccountId> --login
```
...
```bash
npx @subzone81/ms-365-mcp --preset mail
npx @subzone81/ms-365-mcp --list-presets  # See all available presets
```
...
```bash
npx @subzone81/ms-365-mcp --discovery
```
...
The following options can be used when running ms-365-mcp directly from the command line:
...
4. **Configure the server**:
   ```bash
   MS365_MCP_KEYVAULT_URL=https://your-keyvault-name.vault.azure.net npx @subzone81/ms-365-mcp
   ```
...
- Create an [issue](https://github.com/subzone/ms-365-mcp/issues)
- Start a [discussion](https://github.com/subzone/ms-365-mcp/discussions)
- Email: eirikb@eirikb.no
- Discord: https://discord.gg/WvGVNScrAZ or @eirikb

## License

MIT © 2026 Milenko Mitrovic
